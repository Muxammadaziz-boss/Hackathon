...
def index(request):
    ...
    # Tariflar
    tariflar_oddiy = Tariflar_oddiy.objects.all()
    tariflar_pro = Tariflar_pro.objects.all()
    
    # Process description lines for oddiy tariffs
    for tarif in tariflar_oddiy:
        if tarif.description:
            tarif.description_lines = tarif.description.split('\n')
        else:
            tarif.description_lines = []
    
    # Process description lines for pro tariffs  
    for tarif in tariflar_pro:
        if tarif.description:
            tarif.description_lines = tarif.description.split('\n')
        else:
            tarif.description_lines = []
...
def check_telegram(request):
    ...
    try:
        ...
        # Telegram profil sahifasida "If you have Telegram, you can contact @username right away" matni bo'lsa, demak profil bor
        # Agar "View in Telegram" tugmasi bo'lsa ham profil mavjudligini bildiradi
        # Lekin ba'zida profil topilmasa ham "tgme_page_title" bo'lishi mumkin (masalan, "Telegram: Contact @username")
        # Shuning uchun aniqroq tekshiramiz
        has_profile = 'tgme_page_title' in content and username.lower() in content.lower()
